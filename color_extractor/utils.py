...
def calculate_color_statistics(img_array, colors):
    ...
    Args:
        img_array: The image as numpy array
        colors: List of extracted RGB colors
    
    Returns:
        Dictionary with color statistics (percentages, positions, etc.)
    """
    pixels = img_array.reshape(-1, 3).astype(np.float32)
    stats = []
    
    for color in colors:
        # Calculate distances from all pixels to this color
        distances = np.sqrt(np.sum((pixels - color) ** 2, axis=1))
        
        # Find closest pixels (approximate percentage)
        threshold = np.percentile(distances, 20)
        percentage = (np.sum(distances <= threshold) / len(pixels)) * 100
